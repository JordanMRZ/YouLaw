  ...
  <Transition name="toast" appear>
    <div class="lesson-modal settings-modal">
      ...
      <template v-if="!confirmingReset">
        <div class="settings-row theme-setting"><span class="settings-icon">{{ darkMode ? '☀' : '☾' }}</span><div><strong>Modo oscuro</strong><p>{{ darkMode ? 'Usar la apariencia clara.' : 'Usar una apariencia más cómoda para la noche.' }}</p></div><button class="theme-toggle" type="button" :aria-pressed="darkMode" @click="$emit('toggle-dark-mode')"><span>{{ darkMode ? 'Activado' : 'Desactivado' }}</span></button></div>
        <div class="settings-row level-testing-setting"><span class="settings-icon">★</span><div><strong>Probar un nivel</strong><p>Activa un nivel para revisar sus lecciones y preguntas.</p></div><select v-model="selectedTestingLevel" aria-label="Nivel para probar"><option v-for="level in ['A1', 'A2', 'B1', 'B2', 'C1']" :key="level" :value="level">{{ level }}</option></select><button class="theme-toggle" type="button" @click="$emit('set-testing-level', selectedTestingLevel)">Aplicar</button></div>
        <div class="settings-row"><span class="settings-icon">⚙</span><div><strong>Development / Reset</strong><p>Borra el progreso local de YouLaw y vuelve al estado inicial.</p></div></div>
